...
if (typeof document !== "undefined") (() => {
  ...
  function renderRatios(st) {
    ...
    $("#ratio-strip").innerHTML = `
      ...
      <div class="ratio">
        <span class="ratio-label"><abbr title="Proxy pédagogique GAV ÷ NAV. Non assimilable au levier réglementaire AIFM (méthode de l'engagement), qui part de la valeur d'exposition et nette les couvertures.">Levier économique (proxy)</abbr></span>
        <span class="ratio-val mono">${fx2(st.aifm)}</span>
        <span class="ratio-sub">GAV / NAV · repère vs plafond prospectus ${fx2(r.aifmMax)}</span>
        ${pill(st.aifmStatus)}
      </div>
      <div class="ratio">
  ...
  /* Logo Claude (sunburst Anthropic) — 12 rayons, couleur de marque. */
  const CLAUDE_RAYS = Array.from({ length: 12 }, (_, k) =>
    `<rect x="11.1" y="2" width="1.8" height="8.2" rx="0.9" transform="rotate(${k * 30} 12 12)"/>`).join("");
  const sunburst = () => `<svg viewBox="0 0 24 24"><g fill="currentColor">${CLAUDE_RAYS}</g></svg>`;

  const THINK_STEPS = [
    "Lecture des agrégats consolidés (GAV, NAV, LTV, DSCR)",
    "Vérification du statut des covenants",
    "Contrôle du prévisionnel de trésorerie",
    "Calibrage du registre selon l'état du portefeuille",
    "Rédaction du projet de commentaire"
  ];

  function promptLabel(st) {
    if (isZero(st.sliders)) return "Rédige le commentaire de gestion — situation au " + FUND.asOf + ".";
    if (st.globalStatus === "red") return "Rédige le commentaire de gestion — scénario adverse courant (breach covenant).";
    return "Rédige le commentaire de gestion — scénario de stress courant.";
  }

  let chatBusy = false;

  /* Simulation d'une intégration LLM : message utilisateur → réflexion animée →
     réponse en streaming. Le texte est pré-rédigé (commentText), aucune requête réseau. */
  function generateComment() {
    ...
    user.querySelector(".bubble").textContent = promptLabel(st);
    thread.appendChild(user);

    const msg = document.createElement("div");
    msg.className = "msg msg-claude";
    msg.innerHTML = `<span class="msg-avatar">${sunburst()}</span>
      <div class="msg-body">
        <div class="think" id="think"><div class="think-head">Réflexion</div><ul></ul></div>
        <div class="answer"><h4></h4><p></p></div>
      </div>`;
    thread.appendChild(msg);

    const list = msg.querySelector(".think ul");
    const h4 = msg.querySelector(".answer h4");
    const p = msg.querySelector(".answer p");
    ...
    if (reducedMotion) {
      THINK_STEPS.forEach(s => { const li = document.createElement("li"); li.className = "done"; li.textContent = s; list.appendChild(li); });
      h4.textContent = title; p.textContent = text; scroll(); finish(); return;
    }
    ...
    const nextStep = () => {
      ...
      if (i < THINK_STEPS.length) {
        ...
        msg.querySelector("#think").classList.add("collapsed");
        streamAnswer();
      }
    };
    ...
    const streamAnswer = () => {
      ...
      const tick = () => {
        if (w < words.length) {
          caret.insertAdjacentText("beforebegin", (w ? " " : "") + words[w]);
          w++; scroll();
          setTimeout(tick, 14);
        } else { caret.remove(); finish(); }
      };
      tick();
    };
  ...
  /* ============================================================
     Statut covenant (header) + raccourcis clavier
     ============================================================ */
  ...
  function init() {
    ...
    // Recette automatique + réconciliation
